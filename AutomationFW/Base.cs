...
namespace AutomationFW
{
    public static class Base
    {
        ...
        public static void ClickToElement(IWebDriver driver, By locator, string errorMessage = "Object not found", int timer = 10000)
        {
            Actions action = new(driver);
            IWebElement element;
            try
            {
                Wait(driver, timer).Until(de => de.FindElements(locator).Count > 0);
                Wait(driver, timer).Until(de => de.FindElement(locator).Enabled);
        ...
        public static IWebElement FindElement(IWebDriver driver, By locator, string errorMessage = "Object Not Found", int timer = 10000)
        {
            Actions action = new(driver);
            IWebElement elemen;
            try
            {
                Wait(driver, timer).Until(de => FindElements(de, locator).Count > 0);
                elemen = driver.FindElement(locator);
                action.MoveToElement(elemen).Perform();
                return elemen;
            }
            catch
        ...
        public static ReadOnlyCollection<IWebElement> FindElements(IWebDriver driver, By locator) =>
